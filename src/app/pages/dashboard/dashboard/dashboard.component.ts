...
import { SystemMenuService, WeatherService } from '../../../core/services';
...
export class DashboardComponent extends BaseComponent implements OnInit, OnDestroy {
    ...
    private _getWeather(): void{
        const sub = this._weatherService.getWeather().subscribe({
            next: (rsp) => {
                if (!rsp.status || !rsp.data) {
                    return;
                }
                this.weather = rsp.data.main;
                this.weather.temp = Math.round(this.weather.temp);
                this.weather.temp_min = Math.round(this.weather.temp_min);

                if (this.weather.feels_like < this.Constants.HEAT_STROKE_STATUS.WARNING) {
                    this.heatStrokeStatusClass = 'bg-blue';
                    this.heatStrokeColorClass = 'blue';
                } else if (this.weather.feels_like < this.Constants.HEAT_STROKE_STATUS.DANGER) {
                    this.heatStrokeStatusClass = 'bg-yellow';
                    this.heatStrokeColorClass = 'text-yellow';
                } else {
                    this.heatStrokeStatusClass = 'bg-red';
                    this.heatStrokeColorClass = 'text-red';
                }
            },
            error: () => {
                return;
